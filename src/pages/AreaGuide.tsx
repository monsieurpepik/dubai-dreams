...
import { ArrowLeft, TrendingUp, ArrowUpRight, BarChart3 } from 'lucide-react';
...
import { areaDescriptions, areaTags, areaCoordinates, slugToArea } from '@/data/areas';
      ...
      <main className="pt-20">
        <section className="py-16 md:py-24 border-b border-border/30">
          <div className="container-wide">
            <Link to="/areas" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors mb-8">
            ...
            <div className="container-wide">
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="grid grid-cols-2 md:grid-cols-4 gap-8">
                <div>
                  ...
                  <span className="text-2xl font-serif text-foreground">{formatPrice(marketData.avg_price_sqft)}</span>
                </div>
                <div>
                  <span className="label-editorial block mb-2">12-Month Trend</span>
                  <div className="flex items-center gap-2">
                    <TrendingUp className="w-4 h-4 text-accent" />
                    <span className="text-2xl font-serif text-foreground">+{marketData.trend_percentage}%</span>
                  </div>
                </div>
                {marketData.offplan_vs_ready_delta != null && (
                  <div>
                    <span className="label-editorial block mb-2">Off-Plan Discount</span>
                    <span className="text-2xl font-serif text-accent">{marketData.offplan_vs_ready_delta}%</span>
                ...
                <div>
                  ...
                  <span className="text-2xl font-serif text-foreground capitalize">{marketData.trend_12m}</span>
                </div>
              </motion.div>
            </div>
          </section>
                ...
                <div className="max-w-lg space-y-6">
                  <div>
                    ...
                    <div className="h-2 bg-secondary overflow-hidden">
                      <motion.div initial={{ width: 0 }} whileInView={{ width: `${Math.min((marketData.avg_price_sqft / 3500) * 100, 100)}%` }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="h-full bg-accent" />
                    </div>
                  </div>
                  <div>
                    <div className="flex justify-between text-xs text-muted-foreground mb-2">
                      <span>Dubai Average</span>
                      <span>{formatPrice(cityAvgPriceSqft)}/sqft</span>
                    </div>
                    <div className="h-2 bg-secondary overflow-hidden">
                      <motion.div initial={{ width: 0 }} whileInView={{ width: `${Math.min((cityAvgPriceSqft / 3500) * 100, 100)}%` }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.2 }} className="h-full bg-muted-foreground/30" />
                    </div>
                  </div>
                  <p className="text-xs text-muted-foreground">
                    {marketData.avg_price_sqft > cityAvgPriceSqft
                      ? `${areaName} is ${Math.round(((marketData.avg_price_sqft - cityAvgPriceSqft) / cityAvgPriceSqft) * 100)}% above the Dubai average — a premium market.`
                      : `${areaName} is ${Math.round(((cityAvgPriceSqft - marketData.avg_price_sqft) / cityAvgPriceSqft) * 100)}% below the Dubai average — strong value potential.`
                    }
                  </p>
                </div>
              </motion.div>
        ...
        <section className="py-12 md:py-16">
          <div className="container-wide">
            <div className="flex items-center justify-between mb-8">
              <h2 className="font-serif text-2xl md:text-3xl text-foreground">Projects in {areaName}</h2>
              {properties && properties.length > 0 && (
                <span className="text-xs text-muted-foreground uppercase tracking-luxury">{properties.length} {properties.length === 1 ? 'Project' : 'Projects'}</span>
              )}
            </div>
        ...
        <section className="py-12 md:py-16 border-t border-border/10">
          <div className="container-wide max-w-3xl">
            <div className="space-y-4 text-muted-foreground text-sm leading-relaxed">
              <p>
                {areaName} is one of Dubai's most sought-after destinations for off-plan property investment.
                {description} Whether you're a first-time buyer exploring affordable entry points or a seasoned investor
                seeking premium returns, {areaName} offers a range of projects from Dubai's most reputable developers.
              </p>
              <p>
